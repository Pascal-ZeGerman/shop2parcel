...
- [ ] Phase 23: Decouple Stage-2 LLM Extraction from parcelapp POST + honor debug-mode POST suppression — extraction runs independent of quota/debug; POST gated separately (active; bug-fix-driven)
...
- [ ] 23-01-PLAN.md — Wave 0: invert the quota-gate test + add failing AC-1..AC-5/AC-8 test scaffolds
- [ ] 23-02-PLAN.md — Wave 1: add _pending_posts + quota-WARNING throttle flag; persist/hydrate pending_posts store key (no version bump)
- [ ] 23-03-PLAN.md — Wave 2: move quota guard after extractor; debug dry-run early return; quota-defer persistence + throttled WARNING
- [ ] 23-04-PLAN.md — Wave 3: _async_drain_pending_posts + worker-loop wiring + IMAP parity; full-suite/lint/type gate
